...
if os.path.exists( path_to_pidfile ):
    ...
    if pid_is_running:
        logging.info(u'This program is already running, and should not be run twice.')
        sys.exit()
    else:
        os.remove(path_to_pidfile)
        logging.info(u'An old PID file was present, but the program is not running.')
        logging.info(u'Removing the old PID file and running the program anew.')
...
for section in config.sections():
    logging.info(u'Configuring a handler named %s', section)
            ...
            for other_source in messages:
                ...
                for other_message in messagesToWrite[other_source]:
                    ...
                    if match_ratio >= best_match_score:
                        ...
                        best_match_text = (u'   BEST MATCH ON %s: %f  ____ ' % (other_source, match_ratio)) + other_message.content
                        pass
